...
class DatabaseHelper:
 ...
 @staticmethod
 def format_insurance_plan(plan, index=None):
 """Format an insurance plan with full details"""
 response = ""
 if index is not None:
 response += f"**{index}. {plan.get('name', 'Insurance Plan')}**\n"
 else:
 response += f"**{plan.get('name', 'Insurance Plan')}**\n"
 ...
 response += f" Features: Standard coverage\n"
 
 return response
 
 @staticmethod
 def format_doctor_with_slots(doc, index=None):
 """Format a doctor with full details and appointment slots"""
 doc_id = doc.get('doctor_id') or doc.get('id')
 doc_name = doc.get('name', 'N/A')
 specialty = doc.get('specialty', doc.get('doc_type', 'General Medicine'))
 
 # Start with doctor number if index provided
 response = ""
 if index is not None:
 response += f"**{index}. Dr. {doc_name}**\n"
 else:
 response += f"**Dr. {doc_name}**\n"
 ...
 if doc_id:
 try:
 slots = DatabaseHelper.get_availability_slots(doctor_id=doc_id)
 if slots and len(slots) > 0:
 # Group slots by date
 slots_by_date = {}
 for slot in slots[:15]: # Limit to 15 slots
 date = slot.get('date', '')
 time_str = f"{slot.get('start_time', '')[:5]}" if slot.get('start_time') else ''
 if date and time_str:
 if date not in slots_by_date:
 slots_by_date[date] = []
 slots_by_date[date].append(time_str)
 
 if slots_by_date:
 response += f" Available Time Slots:\n"
 for date, times in list(slots_by_date.items())[:5]: # Show up to 5 dates
 date_display = date.split('T')[0] if 'T' in date else date
 # Format date nicely
 try:
 ...
 pass
 times_str = ', '.join(times[:8]) # Show up to 8 times per date
 response += f" • {date_display}: {times_str}\n"
 else:
 response += f" Available Time Slots: Contact for availability\n"
 else:
 response += f" Available Time Slots: Contact for availability\n"
 except Exception as e:
 logging.debug(f"Could not fetch slots for doctor {doc_id}: {e}")
 response += f" Available Time Slots: Contact for availability\n"
 else:
 response += f" Available Time Slots: Contact for availability\n"
...
class AWSBedrockChat(Action):
 ...
 # Build detailed response with full information for each plan
 response = f" **Here are all available insurance plans ({len(insurance_plans)}):**\n\n"
 for i, plan in enumerate(insurance_plans[:5], 1):
 response += DatabaseHelper.format_insurance_plan(plan, index=i)
 response += "\n"
 response += " **Would you like more details about any specific plan?** Just tell me the plan name or number (e.g., '5' or 'Premium Health Plan')!"
 ...
 return [SlotSet("last_shown_insurance_plans", True)] # Set slot to track we showed plans
 else:
 ...
 # Build detailed response with full information for each plan
 response = f" **Here are all available insurance plans ({len(sample_plans)}):**\n\n"
 for i, plan in enumerate(sample_plans, 1):
 response += DatabaseHelper.format_insurance_plan(plan, index=i)
 response += "\n"
 response += " **Would you like more details about any specific plan?** Just tell me the plan name or number (e.g., '5' or 'Premium Health Plan')!"
 safe_dispatcher.utter_message(text=response)
 logging.info(f"PRIORITY 0: Used sample insurance plans as fallback")
 return []
 except Exception as e2:
 logging.error(f"PRIORITY 0: Even sample fallback failed: {e2}")
 safe_dispatcher.utter_message(text="I'm having trouble retrieving insurance plans right now. Please try again in a moment.")
 return []
 else:
 ...
 # PRIORITY 1: Handle specific queries FIRST (before AWS Intelligence)
 msg_lower = user_message.lower()
 
 # Handle number responses (e.g., "5" for insurance plan #5 or doctor #5)
 if msg_lower.strip().isdigit() and len(msg_lower.strip()) <= 2:
 plan_num = int(msg_lower.strip())
 # Check conversation context to see if this refers to insurance plans or doctors
 ...
 # User is referring to insurance plan number
 logging.info(f"Handling number response for insurance plan #{plan_num}")
 insurance_plans = None
 try:
 insurance_plans = DatabaseHelper.get_insurance_plans()
 ...
 if insurance_plans and 1 <= plan_num <= len(insurance_plans):
 plan = insurance_plans[plan_num - 1]
 premium = plan.get('monthly_premium', 'N/A')
 if isinstance(premium, str) and not premium.startswith('$'):
 premium = f"${premium}"
 coverage = plan.get('coverage', 'N/A')
 ...
 deductible = f"${deductible}"
 features = plan.get('features', [])
 features_str = ', '.join(features) if isinstance(features, list) else 'Standard coverage'
 
 response = f" **{plan.get('name', 'Insurance Plan')} (Plan #{plan_num}):**\n\n"
 response += f" **Monthly Premium:** {premium}\n"
 response += f" **Coverage:** {coverage}\n"
 response += f" **Deductible:** {deductible}\n"
 response += f" **Features:** {features_str}\n\n"
 response += "Would you like to enroll in this plan or learn more about it?"
 safe_dispatcher.utter_message(text=response)
 return []
 else:
 response = f"I couldn't find plan #{plan_num}. Please choose a number between 1 and {len(insurance_plans) if insurance_plans else 5}."
 safe_dispatcher.utter_message(text=response)
 return []
 elif "doctor" in context_lower or "specialist" in context_lower or "physician" in context_lower:
 # User is referring to doctor number
 logging.info(f"Handling number response for doctor #{plan_num}")
 # Try to get doctors from context or database
 doctors = None
 try:
 doctors = DatabaseHelper.get_doctors()
 if not doctors:
 ...
 if doctors and 1 <= plan_num <= len(doctors):
 doc = doctors[plan_num - 1]
 response = f" **Dr. {doc.get('name', 'N/A')} (Doctor #{plan_num}):**\n\n"
 ...
 if doc.get('rating'):
 response += f"⭐ **Rating:** {doc.get('rating')}/5\n"
 response += "\n Would you like to book an appointment with Dr. " + doc.get('name', 'N/A') + "? Please provide your preferred date and time!"
 safe_dispatcher.utter_message(text=response)
 return []
 else:
 response = f"I couldn't find doctor #{plan_num}. Please choose a number between 1 and {len(doctors) if doctors else 5}."
 safe_dispatcher.utter_message(text=response)
 return []
 
 # Handle time slot queries
 if any(phrase in msg_lower for phrase in ["time slot", "available time", "time slots", "available slot", "when available", "what times", "what time"]):
 logging.info("Handling time slot query")
 # Check conversation context for doctor information
 context_lower = " ".join([msg.get("content", "").lower() for msg in conversation_history[-5:]])
 
 # Try to get doctor name from context or current message
 doctor_name = None
 if "dr." in msg_lower or "doctor" in msg_lower:
 # Extract doctor name from message
 import re
 match = re.search(r'dr\.?\s*([a-z]+)', msg_lower, re.IGNORECASE)
 if match:
 doctor_name = match.group(1)
 
 # Also check context for doctor names
 if not doctor_name and "dr." in context_lower:
 import re
 match = re.search(r'dr\.?\s*([a-z]+)', context_lower, re.IGNORECASE)
 if match:
 doctor_name = match.group(1)
 
 response = "I can help you check available time slots! "
 if doctor_name:
 response += f"For Dr. {doctor_name.title()}, "
 response += "I can check available appointment times. To get specific time slots, please provide:\n"
 response += "• The doctor's name or number\n"
 response += "• Your preferred date (today, tomorrow, or specific date)\n"
 response += "• Preferred time of day (morning, afternoon, evening)\n\n"
 response += "Would you like me to check available slots for a specific doctor?"
 safe_dispatcher.utter_message(text=response)
 return []
 
 # Handle specific doctor booking requests
 if any(phrase in msg_lower for phrase in ["book with", "book dr.", "appointment with dr.", "schedule with dr.", "want dr.", "need dr."]):
 logging.info("Handling specific doctor booking request")
 # Extract doctor name
 import re
 doctor_name = None
 # Try to extract doctor name from various patterns (including no space after Dr.)
 patterns = [
 r'book\s+(?:with\s+)?dr\.?\s*([a-z]+)',
 r'appointment\s+(?:with\s+)?dr\.?\s*([a-z]+)',
 r'schedule\s+(?:with\s+)?dr\.?\s*([a-z]+)',
 r'want\s+dr\.?\s*([a-z]+)',
 r'need\s+dr\.?\s*([a-z]+)',
 r'dr\.?\s*([a-z]+)', # Handles "Dr. Name" with space
 r'dr\.([a-z]+)', # Handles "Dr.Name" without space
 ]
 ...
 doctors = None
 try:
 all_doctors = DatabaseHelper.get_doctors()
 if all_doctors:
 # Search for doctor by name (case-insensitive)
 doctor_name_lower = doctor_name.lower()
 matching_doctors = [d for d in all_doctors if doctor_name_lower in d.get('name', '').lower()]
 if matching_doctors:
 doctors = matching_doctors
 except Exception as e:
 logging.debug(f"Could not search for doctor: {e}")
 
 if doctors and len(doctors) > 0:
 doc = doctors[0] # Use first match
 response = f"Perfect! I found **Dr. {doc.get('name', doctor_name.title())}**.\n\n"
 response += f" **Specialty:** {doc.get('specialty', 'N/A')}\n"
 response += f" **Department:** {doc.get('department', 'N/A')}\n"
 if doc.get('phone'):
 response += f" **Phone:** {doc.get('phone')}\n"
 response += "\n"
 response += "To complete the booking, please provide:\n"
 response += "• Preferred date (today, tomorrow, or specific date)\n"
 response += "• Preferred time (morning, afternoon, evening, or specific time)\n"
 response += "\n"
 response += "For example: 'Book for tomorrow morning' or 'Schedule for next Monday at 2 PM'"
 else:
 response = f"I'd be happy to help you book an appointment with Dr. {doctor_name.title()}!\n\n"
 response += "To complete the booking, please provide:\n"
 response += "• Preferred date (today, tomorrow, or specific date)\n"
 response += "• Preferred time (morning, afternoon, evening, or specific time)\n"
 response += "\n"
 ...
 response = "I'd be happy to help you book an appointment! Please provide:\n"
 response += "• The doctor's name (e.g., 'Dr. Smith')\n"
 response += "• Preferred date (today, tomorrow, or specific date)\n"
 response += "• Preferred time (morning, afternoon, evening, or specific time)\n"
 ...
 # Handle booking with specific time slot (e.g., "book with Dr. Smith at 2 PM on Monday")
 if any(phrase in msg_lower for phrase in ["book", "schedule", "appointment"]) and any(word in msg_lower for word in ["at", "on", "for", "tomorrow", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]):
 logging.info("Handling booking request with time slot")
 import re
 from datetime import datetime, timedelta
 ...
 # Extract time
 time_match = re.search(r'(\d{1,2})\s*(am|pm|AM|PM)', msg_lower)
 time_str = None
 if time_match:
 ...
 if doc_id and date_str and time_str:
 try:
 slots = DatabaseHelper.get_availability_slots(doctor_id=doc_id, date=date_str)
 available_slot = None
 for slot in slots or []:
 slot_time = slot.get('start_time', '')
 if slot_time and slot_time[:5] == time_str[:5]:
 available_slot = slot
 break
 
 if available_slot:
 response = f" **Appointment Booking Confirmation:**\n\n"
 response += f" **Doctor:** Dr. {doc.get('name', doctor_name.title())}\n"
 response += f" **Specialty:** {doc.get('specialty', 'N/A')}\n"
 response += f" **Date:** {date_obj.strftime('%B %d, %Y') if date_obj else date_str}\n"
 response += f" **Time:** {time_match.group(0).upper()}\n\n"
 response += "To complete the booking, I'll need:\n"
 response += "• Your name and contact information\n"
 response += "• Any symptoms or reason for visit\n\n"
 response += "Would you like to proceed with this appointment? Please provide your details!"
 else:
 response = f"I found Dr. {doc.get('name', doctor_name.title())}, but the requested time slot ({time_match.group(0).upper()} on {date_obj.strftime('%B %d, %Y') if date_obj else date_str}) is not available.\n\n"
 response += "Here are available time slots for Dr. " + doc.get('name', doctor_name.title()) + ":\n"
 # Get available slots
 slots = DatabaseHelper.get_availability_slots(doctor_id=doc_id, date=date_str)
 if slots:
 for slot in slots[:5]:
 slot_time = slot.get('start_time', '')
 if slot_time:
 response += f"• {slot_time[:5]}\n"
 else:
 response += "Please contact the doctor's office for availability.\n"
 response += "\nWould you like to choose a different time?"
 except Exception as e:
 logging.debug(f"Could not check slot availability: {e}")
 response = f" I found Dr. {doc.get('name', doctor_name.title())}!\n\n"
 response += f" **Requested:** {date_obj.strftime('%B %d, %Y') if date_obj else date_str} at {time_match.group(0).upper() if time_match else 'specified time'}\n\n"
 response += "To complete the booking, please provide:\n"
 response += "• Your name and contact information\n"
 response += "• Any symptoms or reason for visit\n\n"
 ...
 response = f" I found Dr. {doc.get('name', doctor_name.title())}!\n\n"
 if not date_str:
 response += "Please specify a date (e.g., 'tomorrow', 'Monday', or a specific date).\n"
 if not time_str:
 response += "Please specify a time (e.g., '2 PM', '10 AM').\n"
 response += "\nFor example: 'Book with Dr. " + doc.get('name', doctor_name.title()) + " at 2 PM on Monday'"
 else:
 response = "I'd be happy to help you book an appointment!\n\n"
 response += "To book a specific time slot, please provide:\n"
 response += "• Doctor's name (e.g., 'Dr. Smith')\n"
 response += "• Date (e.g., 'tomorrow', 'Monday', or specific date)\n"
 response += "• Time (e.g., '2 PM', '10 AM')\n\n"
 response += "For example: 'Book with Dr. Smith at 2 PM on Monday'"
 
 safe_dispatcher.utter_message(text=response)
 return []
 
 # PRIORITY 0: Handle numbered responses for insurance plan selection
 # Check if user entered a number (1-18) in response to insurance plan listing
 if msg_lower.strip().isdigit():
 plan_number = int(msg_lower.strip())
 
 # Check if we recently showed insurance plans using slot
 showed_plans_slot = tracker.get_slot("last_shown_insurance_plans")
 
 # Also check multiple sources for insurance context
 # 1. Check conversation history
 context_lower = " ".join([msg.get("content", "").lower() for msg in conversation_history[-5:]])
 
 # 2. Check the last bot event
 last_bot_text = ""
 for event in reversed(tracker.events[-10:]):
 if event.get("event") == "bot":
 last_bot_text = event.get("text", "").lower()
 break
 
 # Detect insurance context
 is_insurance_context = (
 showed_plans_slot == True or # We explicitly set this slot
 "insurance" in context_lower or 
 "plan" in context_lower or 
 "coverage" in context_lower or
 "insurance" in last_bot_text or
 "monthly premium" in last_bot_text or
 "would you like more details" in last_bot_text or
 "premium health plan" in last_bot_text or
 "basic health plan" in last_bot_text
 )
 
 logging.info(f"Numbered input: {plan_number}, Slot: {showed_plans_slot}, Insurance context: {is_insurance_context}")
 logging.info(f"Last bot text: {last_bot_text[:150] if last_bot_text else 'None'}")
 
 if is_insurance_context and 1 <= plan_number <= 20:
 logging.info(f"Detected numbered response ({plan_number}) for insurance plan selection")
 
 try:
 # Get all insurance plans
 insurance_plans = DatabaseHelper.get_insurance_plans()
 
 if insurance_plans and 1 <= plan_number <= len(insurance_plans):
 selected_plan = insurance_plans[plan_number - 1]
 
 # Build detailed response for the selected plan
 response = f" **Details for Plan #{plan_number}: {selected_plan.get('plan_name')}**\n\n"
 
 # Price information
 monthly_premium = selected_plan.get('monthly_premium', 0)
 deductible = selected_plan.get('deductible', 0)
 coverage = selected_plan.get('coverage_percentage', 0)
 
 response += f" **Pricing:**\n"
 response += f" • Monthly Premium: ${monthly_premium:.2f}\n"
 response += f" • Annual Deductible: ${deductible:.2f}\n"
 response += f" • Coverage: {coverage}%\n\n"
 
 # Features
 features = selected_plan.get('features', [])
 if features:
 response += f" **Features & Benefits:**\n"
 for feature in features:
 response += f" ✓ {feature}\n"
 response += "\n"
 
 # Comparison with other plans
 response += f" **How it Compares:**\n"
 if monthly_premium < 200:
 response += " • Budget-friendly option with essential coverage\n"
 elif monthly_premium < 350:
 response += " • Mid-range plan with comprehensive benefits\n"
 else:
 response += " • Premium plan with extensive coverage\n"
 
 if coverage >= 90:
 response += " • Excellent coverage percentage\n"
 elif coverage >= 80:
 response += " • Good coverage percentage\n"
 
 response += "\n **Next Steps:**\n"
 response += " • Would you like to enroll in this plan?\n"
 response += " • Compare with another plan? (Enter another number)\n"
 response += " • See all plans again? (Type 'show all plans')\n"
 
 safe_dispatcher.utter_message(text=response)
 logging.info(f"Showed details for insurance plan #{plan_number}")
 # Clear the slot after showing plan details
 return [SlotSet("last_shown_insurance_plans", False)]
 else:
 response = f"I don't have a plan #{plan_number}. We have {len(insurance_plans) if insurance_plans else 18} insurance plans available.\n\n"
 response += "Please enter a number between 1 and " + str(len(insurance_plans) if insurance_plans else 18) + ", or ask me to 'show all insurance plans'."
 safe_dispatcher.utter_message(text=response)
 return []
 
 except Exception as e:
 logging.error(f"Error handling numbered insurance plan selection: {e}")
 response = "I'm having trouble retrieving that plan's details. Could you try asking 'show insurance plans' to see all available options?"
 safe_dispatcher.utter_message(text=response)
 return []
 ...
 # Also check for appointment booking context
 is_doctor_context = any(word in context_lower for word in ["doctor", "specialist", "physician", "suggest", "search", "available", "appointment", "book", "schedule", "would you like"])
 is_lab_context = any(word in context_lower for word in ["lab", "result", "test", "medical record", "report"])
 
 if is_doctor_context and not is_lab_context:
 ...
 if doctors and len(doctors) > 0:
 # Build detailed response with full information and appointment slots for each doctor
 response = f" I found {len(doctors)} doctor(s) available:\n\n"
 for i, doc in enumerate(doctors[:5], 1):
 response += DatabaseHelper.format_doctor_with_slots(doc, index=i)
 response += "\n"
 response += " **Would you like to book an appointment with any of these doctors?** Just tell me the doctor's name or number, and your preferred date/time (e.g., 'Book with Dr. Smith at 2 PM on Monday' or 'Book doctor #1 for tomorrow at 10 AM')!"
 # Ensure single message by sending as one string
 safe_dispatcher.utter_message(text=response)
 logging.info(f"action_aws_bedrock_chat: Handled 'yes' for doctors, found {len(doctors)} doctors - sent as SINGLE message")
 return []
 else:
 ...
 if insurance_plans and len(insurance_plans) > 0:
 # Build detailed response with full information for each plan
 response = f" Here are all available insurance plans ({len(insurance_plans)} plans):\n\n"
 for i, plan in enumerate(insurance_plans[:5], 1):
 response += DatabaseHelper.format_insurance_plan(plan, index=i)
 response += "\n"
 response += " **Would you like more details about any specific plan?** Just tell me the plan name or number (e.g., '5' or 'Premium Health Plan')!"
 ...
 # Set slot to indicate we just showed insurance plans
 return [SlotSet("last_shown_insurance_plans", True)]
 else:
 ...
 # Build detailed response with full information and appointment slots
...
Would you like more details about any specific plan, or personalized recommendations?"""
 ...
 if doctors and len(doctors) > 0:
 # Build compact single-line response to prevent splitting
 response = f" I found {len(doctors)} doctor(s) available:\n"
 for i, doc in enumerate(doctors[:5], 1):
 ...
 # Single line per doctor to prevent splitting
 response += f"**{i}. Dr. {doc.get('name', 'N/A')}** - {specialty}"
 if phone:
 response += f" - {phone}"
 if rating:
 response += f" - ⭐ {rating}/5"
 if exp:
 response += f" - {exp} years exp"
 response += "\n"
 response += "\n Would you like to book an appointment with any of these doctors? Just tell me the doctor's name or number!"
 else:
 # ALWAYS use sample doctors if database fails
 ...
 response += " **URGENT:** This appears to be an emergency. Please seek immediate medical care or call 911.\n\n"
 elif urgency == 'urgent':
 response += " **URGENT:** This should be addressed soon. I can help you find urgent care or schedule an appointment.\n\n"
