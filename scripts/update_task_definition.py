...
def update_task_definition(task_def_json, config=None):
 ...
 print(" WARNING: DB_PASSWORD not found. Using placeholder. Set via environment or AWS Secrets Manager.", file=sys.stderr)
 db_password = "REPLACE_WITH_SECRET"
 
 if not mongodb_uri or mongodb_uri is None:
 print(" WARNING: MONGODB_URI not found. Using placeholder. Set via environment or AWS Secrets Manager.", file=sys.stderr)
 ...
 print(" Flask wrapper container not found", file=sys.stderr)
 ...
 if not rasa_updated:
 print(" Rasa backend container not found", file=sys.stderr)
 ...
 print(" WARNING: POSTGRES_ENDPOINT not found. Using placeholder.", file=sys.stderr)
